...
def check_shape_contents(list_in: list) -> bool:
    '''
    Receives a list of lists or strings, and checks to see if its shape
    and contents are appropriate for a Sudoku board

    Arguments:
    ==========
    list_in (list) - a list of 9 lists or strings, each of which has len of 9

    Returns:
    ========
    True if shape and contents are correct
    False if either shape or contents is incorrect
    '''
...
def preprocess(list_in: list) -> np.array:
    ...
    list_array = list(
        map(
            lambda row: [int(str(a)) if str(a).isnumeric() else np.nan for a in row],
            list_in
        )
    )
